...
@transient()
@autoinject()
export class StorageBulkGenericAssignment implements IStorageReaderOperations {
  ...
  public async confirm() {
    ...
    if (this.canUpdateDb) {
      ...
      // Add to database
      await Server.Http_AddItem(this.apiPath, this.item).then(data => {
        ...
        this.controller.close(true, { "primaryContainer": this.primaryContainer, "materials": this.materials, "material": this.material });
      }).catch(error => {
  ...
  async tagIdentIsValid(): Promise<boolean> {
    ...
    const tagIdent = await Reader_Ident(this.storageOperations.reader.readerId, this.storageOperations.newTags[0].UID[0]).catch(error => {
